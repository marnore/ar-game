...
namespace MaterialUI
{
	public class RippleConfig : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler
	{
		...
		[HideInInspector()]
		public float sizePercentage = 100f;
		[HideInInspector()]
		...
		[HideInInspector()]
		public float rippleStartAlpha = 0.4f;
		[HideInInspector()]
		public float rippleEndAlpha = 0.2f;
		...
		void Start()
		{
			if (toggleMask)
			{
				if (gameObject.GetComponent<RectMask2D>())
					thisMask = gameObject.GetComponent<RectMask2D>();
				else
				{
					thisMask = gameObject.AddComponent<RectMask2D>();
